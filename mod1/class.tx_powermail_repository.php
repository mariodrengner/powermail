...
class tx_powermail_repository {
	...
	public function main() {
		...
		$where = '
			pid = ' . intval($this->pid) . $startDateAdd . $endDateAdd . '
			AND hidden = 0
			AND deleted = 0';
		...
		if ($res2 !== FALSE) {
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res2)) {
				$i++;
				$this->ajaxContentArray['rows'][] = array('id' => $i, 'uid' => $row['uid'], 'crdate' => date($this->timeformat, $row['crdate']), 'sender' => $row['sender'], 'recipient' => $row['recipient'], 'senderIP' => $row['senderIP'], 'piVars' => $this->transformPiVars($row['piVars']));
			}
	...
	protected function transformPiVars($piVars) {
		if (!is_array(t3lib_div::xml2array($piVars))) {
			return t3lib_div::xml2array($piVars);
		}
	...
	public function getLabelsAndFormtypes() {
		...
		if ($res !== FALSE) {
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				$labels['labels'][] = array('uid' => $row['uid'], 'title' => $row['title'], 'formtype' => $row['formtype']);
				$i++;
			}
			$labels['results'] = $i;
			$labels['success'] = TRUE;
