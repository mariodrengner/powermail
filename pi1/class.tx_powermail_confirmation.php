 ...
 *  Copyright notice
 *
 *  (c) 2012 powermail development team (details on http://forge.typo3.org/projects/show/extension-powermail)
 *  All rights reserved
 *
...
class tx_powermail_confirmation extends tslib_pibase {
	...
	function main($conf, $sessionfields, $cObj) {
		...
		$additionalParams = '';
		// Generate last page for multiple sites
		if ($this->cObj->data['tx_powermail_multiple'] == 2 && $this->cObj->data['tx_powermail_fieldsets'] > 1) {
			$additionalParams = '&tx_powermail_pi1[multiple]=' . $this->cObj->data['tx_powermail_fieldsets'] .
		...
		$this->markerArray['###POWERMAIL_TARGET_SUBMIT###'] = $this->cObj->typolink('x', array(
			...
			'additionalParams' => '&tx_powermail_pi1[mailID]=' .
				(($this->cObj->data['_LOCALIZED_UID'] > 0) ? $this->cObj->data['_LOCALIZED_UID'] : $this->cObj->data['uid']) .
