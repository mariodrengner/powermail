...
class tx_powermail_html extends tslib_pibase {
	...
	public function main($conf, $sessionfields, $cObj, $row, $tabindex, $counter = 0) {
		...
		if (!$this->div->subpartsExists($this->tmpl)) {
			$this->content = $this->pi_getLL('error_templateNotFound', 'Template not found, check path to your powermail templates') . '<br />';
		}
	...
	private function html_content() {
		...
		$conf = array( // config
			'tables' => 'tt_content', 'source' => $uid, 'dontCheckPid' => 1);
	...
	private function html_countryselect() {
		...
		if (t3lib_extMgm::isLoaded('static_info_tables', 0)) { // only if static_info_tables is loaded
			...
			$select = '*' . $localfield; //'uid, cn_iso_2, cn_short_local, cn_short_en' . $localfield,
			...
			if ($res !== FALSE) { // If there is a result
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every country
					if ($row['cn_short_lang']) {
						$row['cn_short_lang'] = $this->div->charset($row['cn_short_lang'], $this->conf['countryselect.']['charset']);
					} // change charset of value
					...
					// Preselection
					if ($row['uid'] == $this->pi_getFFvalue(t3lib_div::xml2array($this->xml), 'preselect') && $this->pi_getFFvalue(t3lib_div::xml2array($this->xml), 'preselect') > 0) {
						$markerArray['###SELECTED###'] = ' selected="selected"';
					} // preselect one country
					else {
						$markerArray['###SELECTED###'] = '';
					}
	...
	private function setGlobalMarkers() {
		...
		if (!empty($this->conf['barrier-free.']['accesskey'])) { // If manually set accesskey in ts
			...
			for ($i = 0; $i < count($array); $i++) { // one loop for every uid/accesskey part
				...
				if ($this->accesskeyarray[$i][1] != '') {
					$this->newaccesskey[$this->accesskeyarray[$i][0]][$this->accesskeyarray[$i][1]] = $this->accesskeyarray[$i][2];
				} // array for radiobuttons and checkboxes
