...
class tx_powermail_mandatory extends tslib_pibase {
	...
	public function main($conf, $sessionfields, $cObj) {
		...
		if ($this->conf['mandatory.']['messages']) {
			$this->tmpl['mandatory']['all'] = str_replace($this->tmpl['mandatory']['backbutton'], '', $this->tmpl['mandatory']['all']);
		} // clear BACKBUTTON subpart if not needed
					...
					if (!is_array($this->sessionfields['uid' . $row['uid']])) { // first level
						...
						if (isset($this->sessionfields['uid' . $row['uid']])) {
							...
							foreach ($this->sessionfields['uid' . $row['uid']] as $key => $value) { // one loop for every field
								if ($this->sessionfields['uid' . $row['uid']][$key] != '') {
									$error = 0;
								} // set error
							}
							if ($error) {
		...
		if (is_array($uniquearray) && isset($uniquearray)) {
			foreach ($uniquearray as $value) {
				...
				if (is_numeric(str_replace('uid', '', strtolower($value))) && $this->sessionfields[strtolower($value)]) { // like uid11 and value exists
					...
					$orderBy = '';
					$limit = '1';
					// get any entry with same IP address like current user
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery($select, $from, $where, $groupBy, $orderBy, $limit);
					if ($res !== FALSE) {
	...
	public function captchaCheck() {
		if ( // only if a supported captcha extension is loaded
			t3lib_extMgm::isLoaded('captcha', 0) || t3lib_extMgm::isLoaded('sr_freecap', 0) || t3lib_extMgm::isLoaded('jm_recaptcha', 0) || t3lib_extMgm::isLoaded('wt_calculating_captcha', 0)) {
			...
			if ($res !== FALSE) { // If there is a result
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every captcha field
					...
					if (t3lib_extMgm::isLoaded('sr_freecap', 0) && $this->conf['captcha.']['use'] == 'sr_freecap') { // use sr_freecap if available
						...
						} elseif (($_SESSION['sr_freecap_word_hash'] != md5($this->sessionfields['uid' . $row['uid']])) && ($_SESSION['sr_freecap_word_hash'] != md5($this->sessionfields['uid' . $row['uid']] . "\n")) && ($GLOBALS['TSFE']->fe_user->sesData['tx_sr_freecap']['sr_freecap_word_hash'] != md5($this->sessionfields['uid' . $row['uid']])) && (is_object($this->freeCap) && !$this->freeCap->checkWord($this->sessionfields['uid' . $row['uid']]))
						) {
