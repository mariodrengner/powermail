			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				switch ($row['CType']) {
					...
					case 'shortcut':
						...
						$shortcutSelect = 'uid';
						$shortcutFrom = 'tt_content';
						$shortcutWhere = 'uid IN ( ' . $recordUids . ' ) AND CType = "powermail_pi1"' . $ttContentWhere;
						$shortcutGroupBy = '';
						$shortcutOrderBy = '';
						$shortcutLimit = '1';
						$shortcutRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
							$shortcutSelect, $shortcutFrom, $shortcutWhere, $shortcutGroupBy, $shortcutOrderBy, $shortcutLimit
						);
						if ($shortcutRes !== FALSE) {
