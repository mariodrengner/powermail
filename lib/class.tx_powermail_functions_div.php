...
class tx_powermail_functions_div {
	 ...
	 * @return array Filtered piVars Array
	 */
	public function sec($array) {
		...
		if (isset($array) && is_array($array)) {
				// One loop for every key in first level
			foreach ($array as $key => $value) {
					// All others piVars than UID34
				if (!is_numeric(str_replace('UID', '', $key)) && !is_array($value)) {
	...
	public function clearName($string, $strtolower = 0, $cut = 0) {
		...
		} // string to lower if active
		if ($cut) {
			$string = substr($string, 0, $cut);
		} // cut after X signs if active
	...
	public function makePlain($content) {
		...
		$htmltagarray = array( // This tags will be added with linebreaks
			'</p>', '</tr>', '</li>', '</h1>', '</h2>', '</h3>', '</h4>', '</h5>', '</h6>', '</div>', '</legend>', '</fieldset>', '</dd>', '</dt>');
		$notallowed = array( // This array contains not allowed signs which will be removed
			'&nbsp;', '&szlig;', '&Uuml;', '&uuml;', '&Ouml;', '&ouml;', '&Auml;', '&auml;',);
